...
- **register_type** (*Optional*):
  - `coil`  : stores a binary number in the device with *Function Code 5 or 15*. A coil is a 16 bit register but any number other than zero gets stored as a one. This is equivalent to how C++ treats booleans.
  - `holding`  : stores the number on a holding register in the device with *Function Code 6 or 16*.
